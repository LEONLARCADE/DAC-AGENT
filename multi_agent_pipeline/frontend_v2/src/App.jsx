...
export default function DacAgentApp() {
  ...
  const [phase, setPhase] = useState("intro"); // intro → opening → welcome → configure → app
  ...
  const send = useCallback(async (textToProcess) => {
    const txt = (textToProcess || chatInput).trim();
    if (!txt) return;
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/run-pipeline", {
        ...
        body: JSON.stringify({ topic: txt, loop_count: loopCount, paper_count: paperCount })
      });

      if (!response.ok) throw new Error("Pipeline returned an error");
      const data = await response.json();

      const finalR = data.final_result;
      let formattedStr = "Processing complete.";

      if (finalR) {
        formattedStr = Object.entries(finalR)
          .filter(([k]) => k !== 'previous_data' && k !== 'agent')
          .map(([k, v]) => {
      ...
      setMsgs(m => [...m, { r: "bot", t: formattedStr }]);
    } catch (err) {
      setMsgs(m => [...m, { r: "bot", t: `System Error: ${err.message}` }]);
    }
  ...
  const txt = (op = 0.9, sz = 13, w = 400) => ({ fontFamily: SF, fontSize: sz, fontWeight: w, color: `rgba(255,255,255,${op})`, letterSpacing: "-0.01em" });
  const label = { fontFamily: SF, fontSize: 10, fontWeight: 600, color: "rgba(255,255,255,0.3)", letterSpacing: "0.09em", textTransform: "uppercase" };
  ...
  const ChatScreen = () => (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        ...
        {msgs.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.r === "user" ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 10 }}>
            ...
            <div style={{
              ...
              ...txt(0.9, 14, 400), lineHeight: 1.7, fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", whiteSpace: "pre-wrap"
            }}>{m.t}</div>
          </div>
        ))}
        {loading && (
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px", flexShrink: 0 }}>
        <input
          value={chatInput}
          onChange={(e) => setChatInput(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && send()}
          placeholder="Command override..."
          style={{ flex: 1, background: "rgba(255,255,255,0.06)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", border: "1px solid rgba(139,92,246,0.3)", borderRadius: 14, padding: "14px 20px", color: "rgba(255,255,255,0.85)", fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "0.9rem", outline: "none", boxShadow: "0 0 0 1px rgba(139,92,246,0.1) inset" }}
        />
        <button onClick={() => send()} disabled={loading} style={{ width: 44, height: 44, borderRadius: 12, background: "rgba(147,51,234,0.55)", border: "1px solid rgba(192,132,252,0.35)", borderTop: "1px solid rgba(220,180,255,0.5)", cursor: loading ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(12px)", boxShadow: "0 0 14px rgba(147,51,234,0.4)" }}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(220,180,255,0.95)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
        </button>
      </div>
    </div>
  );
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        ::placeholder{color:rgba(255,255,255,0.25);font-family:'Yuji Syuku','Kaisei HarunoUmi',serif;letter-spacing:0.06em}
        input,button{font-family:'Yuji Syuku','Kaisei HarunoUmi',serif}
      ...
      {phase === "intro" && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", animation: "introFadeIn 0.8s ease both" }}>
          {[...Array(18)].map((_, i) => (
            <div key={i} style={{ position: "absolute", left: `${10 + Math.random() * 80}%`, top: `${20 + Math.random() * 60}%`, width: i % 3 === 0 ? 3 : 2, height: i % 3 === 0 ? 3 : 2, borderRadius: "50%", background: i % 2 === 0 ? "rgba(192,132,252,0.7)" : "rgba(100,200,255,0.5)", "--dx": `${(Math.random() - .5) * 60}px`, animation: `particleDrift ${2 + Math.random() * 3}s ease-in ${Math.random() * 2}s infinite`, pointerEvents: "none" }} />
          ))}
          <div style={{ position: "absolute", top: "8%", left: "6%", fontSize: "8rem", color: "rgba(192,132,252,0.08)", fontFamily: "'Yuji Syuku',serif", animation: "kanji1 4s ease-in-out infinite", pointerEvents: "none", userSelect: "none" }}>武</div>
          <div style={{ position: "absolute", bottom: "10%", right: "6%", fontSize: "6rem", color: "rgba(192,132,252,0.06)", fontFamily: "'Yuji Syuku',serif", animation: "kanji2 5s ease-in-out 1s infinite", pointerEvents: "none", userSelect: "none" }}>道</div>
          <div style={{ position: "absolute", top: "15%", right: "10%", fontSize: "4rem", color: "rgba(147,51,234,0.07)", fontFamily: "'Yuji Syuku',serif", animation: "kanji1 6s ease-in-out 2s infinite", pointerEvents: "none", userSelect: "none" }}>侍</div>
          <div style={{ position: "absolute", bottom: "20%", left: "9%", fontSize: "5rem", color: "rgba(147,51,234,0.05)", fontFamily: "'Yuji Syuku',serif", animation: "kanji2 4.5s ease-in-out 0.5s infinite", pointerEvents: "none", userSelect: "none" }}>龍</div>
            ...
            <svg width="120" height="120" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
              <defs>
                <mask id="dm">
                  ...
                  {/* arrow slot cut horizontally through D */}
                  <rect x="0" y="89" width="200" height="22" fill="black" />
                  {/* left arrowhead notch */}
                  <polygon points="52,82 22,100 52,118" fill="black" />
                  {/* right arrowhead notch */}
                  <polygon points="148,82 178,100 148,118" fill="black" />
                </mask>
              </defs>
              {/* D body */}
              <path d="M50 28 L72 28 Q140 28 172 72 Q192 88 192 100 Q192 112 172 128 Q140 172 72 172 L50 172 Z" fill="#b0aaf5" mask="url(#dm)" />
              {/* Arrow shaft */}
          ...
          <div style={{ fontFamily: "'Yuji Syuku',serif", fontSize: "0.75rem", letterSpacing: "0.55em", color: "rgba(192,132,252,0.45)", marginBottom: 42, textTransform: "uppercase" }}>✦ &nbsp; Autonomous AI Ronin &nbsp; ✦</div>
          <button onClick={handleEnter} style={{ position: "relative", overflow: "hidden", padding: "28px 72px", borderRadius: 20, border: "1px solid rgba(192,132,252,0.35)", borderTop: "1px solid rgba(220,190,255,0.55)", background: "rgba(255,255,255,0.07)", backdropFilter: "blur(40px) saturate(200%)", WebkitBackdropFilter: "blur(40px) saturate(200%)", cursor: "pointer", animation: "introPulse 2.8s ease-in-out infinite, floatBtn 3.5s ease-in-out infinite", boxShadow: "0 0 40px rgba(168,85,247,0.55),0 0 100px rgba(147,51,234,0.28),inset 0 1px 0 rgba(255,255,255,0.22)" }}>
            ...
            <div style={{ fontFamily: "'Yuji Syuku',serif", fontSize: "clamp(1.8rem,4vw,3rem)", fontWeight: 700, letterSpacing: "0.28em", color: "#fff", textShadow: "0 0 24px rgba(192,132,252,0.9),0 0 60px rgba(147,51,234,0.6)", position: "relative", zIndex: 1, userSelect: "none" }}>DAC AGENT</div>
          </button>
          <div style={{ marginTop: 40, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ fontFamily: "'Yuji Syuku',serif", fontSize: "0.68rem", letterSpacing: "0.35em", color: "rgba(192,132,252,0.35)" }}>TOUCH TO ENTER</div>
          </div>
        </div>
      )}
      ...
      {phase === "opening" && (
        <div style={{ position: "fixed", inset: 0, zIndex: 60, pointerEvents: "none" }}>
          ...
          <div style={{ position: "absolute", top: 0, left: 0, bottom: 0, width: "50%", background: "rgba(8,2,18,0.96)", backdropFilter: "blur(2px)", animation: "doorLeft 1.7s cubic-bezier(0.7,0,0.85,1) 0.1s both", zIndex: 61, overflow: "hidden" }}>
            ...
            <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", fontSize: "5rem", color: "rgba(192,132,252,0.12)", fontFamily: "'Yuji Syuku',serif", userSelect: "none" }}>武</div>
          </div>
          <div style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: "50%", background: "rgba(8,2,18,0.96)", backdropFilter: "blur(2px)", animation: "doorRight 1.7s cubic-bezier(0.7,0,0.85,1) 0.1s both", zIndex: 61, overflow: "hidden" }}>
            ...
            <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", fontSize: "5rem", color: "rgba(192,132,252,0.12)", fontFamily: "'Yuji Syuku',serif", userSelect: "none" }}>道</div>
          </div>
      ...
      {phase === "welcome" && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", animation: "welcomeFadeIn 0.7s ease both" }}>
          ...
          {/* Subtle network dot grid background */}
          {[...Array(14)].map((_, i) => (
            <div key={i} style={{ position: "absolute", width: 3, height: 3, borderRadius: "50%", background: "rgba(168,85,247,0.25)", left: `${8 + i * 6.5}%`, top: `${15 + ((i * 47) % 70)}%`, boxShadow: "0 0 6px rgba(168,85,247,0.3)", animation: `nodeFloat ${3 + i * .4}s ease-in-out ${i * .3}s infinite`, pointerEvents: "none" }} />
          ))}
          {/* Connector lines */}
          <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", opacity: 0.12 }}>
            {[[10, 25, 30, 45], [30, 45, 55, 30], [55, 30, 75, 55], [75, 55, 90, 35], [20, 65, 45, 75], [45, 75, 70, 60]].map(([x1, y1, x2, y2], i) => (
              <line key={i} x1={`${x1}%`} y1={`${y1}%`} x2={`${x2}%`} y2={`${y2}%`} stroke="rgba(168,85,247,0.8)" strokeWidth="1" />
            ))}
          </svg>

          {/* Icon orb with DAC logo */}
          <div style={{ width: 90, height: 90, borderRadius: "50%", background: "rgba(10,5,20,0.85)", border: "2px solid rgba(139,92,246,0.6)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 28, boxShadow: "0 0 30px rgba(139,92,246,0.4), 0 0 60px rgba(139,92,246,0.15)", animation: "nodeFloat 3s ease-in-out infinite" }}>
            <svg width="52" height="52" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
              ...
              <path d="M50 28 L72 28 Q140 28 172 72 Q192 88 192 100 Q192 112 172 128 Q140 172 72 172 L50 172 Z" fill="#b0aaf5" mask="url(#wdm)" />
              <rect x="22" y="89" width="156" height="22" fill="#b0aaf5" />
              <polygon points="22,100 52,80 52,120" fill="#b0aaf5" />
              <polygon points="178,100 148,80 148,120" fill="#b0aaf5" />
            </svg>
          ...
          <div style={{ textAlign: "center", marginBottom: 10 }}>
            <div style={{ fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "clamp(1rem,2.5vw,1.4rem)", fontWeight: 400, color: "rgba(255,255,255,0.85)", letterSpacing: "0.06em", marginBottom: 8 }}>Welcome To</div>
            <div style={{ fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "clamp(1.8rem,4vw,3rem)", fontWeight: 700, letterSpacing: "0.12em", background: "linear-gradient(135deg,#818cf8,#a855f7,#c084fc)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", lineHeight: 1.2 }}>DAC AGENT</div>
          </div>

          {/* Subtitle */}
          <div style={{ fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "0.88rem", color: "rgba(255,255,255,0.38)", letterSpacing: "0.04em", marginBottom: 36, textAlign: "center" }}>
          ...
          <div style={{ width: "min(540px,80vw)", display: "flex", gap: 10, marginBottom: 16, position: "relative" }}>
            <input
              ...
              onKeyDown={e => { if (e.key === "Enter") setPhase("app"); }}
              placeholder="Enter research topic..."
              style={{ flex: 1, background: "rgba(255,255,255,0.06)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", border: "1px solid rgba(139,92,246,0.3)", borderRadius: 14, padding: "14px 54px 14px 20px", color: "rgba(255,255,255,0.85)", fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "0.9rem", outline: "none", boxShadow: "0 0 0 1px rgba(139,92,246,0.1) inset" }}
            />
            <button onClick={() => setPhase("app")} style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", width: 38, height: 38, borderRadius: 10, background: "rgba(147,51,234,0.55)", border: "1px solid rgba(192,132,252,0.35)", borderTop: "1px solid rgba(220,180,255,0.5)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(12px)", boxShadow: "0 0 14px rgba(147,51,234,0.4)" }}>
          ...
          {/* Next Step button */}
          <button onClick={() => {
            if (welcomeInput.trim() !== "") setPhase("configure");
          }} style={{ width: "min(540px,80vw)", padding: "15px", borderRadius: 999, background: "rgba(147,51,234,0.25)", backdropFilter: "blur(30px) saturate(180%)", WebkitBackdropFilter: "blur(30px) saturate(180%)", border: "1px solid rgba(192,132,252,0.35)", borderTop: "1px solid rgba(220,180,255,0.5)", color: "rgba(220,180,255,0.95)", fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "1rem", fontWeight: 600, letterSpacing: "0.12em", cursor: "pointer", boxShadow: "0 0 0 0.5px rgba(192,132,252,0.15) inset, 0 8px 32px rgba(147,51,234,0.3)", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
      ...
      {phase === "configure" && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", animation: "welcomeFadeIn 0.7s ease both" }}>
          ...
          {/* Subtle network dot grid background */}
          {[...Array(14)].map((_, i) => (
            <div key={`cfg-bg-${i}`} style={{ position: "absolute", width: 3, height: 3, borderRadius: "50%", background: "rgba(168,85,247,0.25)", left: `${8 + i * 6.5}%`, top: `${15 + ((i * 47) % 70)}%`, boxShadow: "0 0 6px rgba(168,85,247,0.3)", animation: `nodeFloat ${3 + i * .4}s ease-in-out ${i * .3}s infinite`, pointerEvents: "none" }} />
          ))}

          <div style={{ ...liqCard, width: "min(580px,88vw)", padding: "40px 40px 36px", borderRadius: 24, position: "relative", overflow: "hidden", boxShadow: "0 0 0 1px rgba(255,255,255,0.1) inset, 0 0 80px rgba(147,51,234,0.2), 0 40px 80px rgba(0,0,0,0.55)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 28 }}>
              ...
              <div>
                {/* Bold title at default 14px size */}
                <div style={{ fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "14px", fontWeight: 700, color: "rgba(255,255,255,0.93)", letterSpacing: "0.07em", textTransform: "uppercase", textShadow: "0 0 18px rgba(192,132,252,0.45)" }}>
                  Configure Your AI Agents
                </div>
                <div style={{ fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "14px", color: "rgba(255,255,255,0.30)", marginTop: 5 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginBottom: 28 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <label style={{ fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "14px", fontWeight: 700, color: "rgba(255,255,255,0.78)", letterSpacing: "0.03em" }}>Custom Loop Count <span style={{ color: "rgba(255,100,100,0.85)" }}>*</span></label>
                </div>
                <input
                  type="number"
                  required
                  ...
                  style={{ width: "100%", background: "rgba(255,255,255,0.06)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", border: "1px solid rgba(139,92,246,0.3)", borderRadius: 14, padding: "14px 20px", color: "rgba(255,255,255,0.9)", fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "14px", outline: "none", boxShadow: "0 0 0 1px rgba(139,92,246,0.1) inset" }}
                />
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <label style={{ fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "14px", fontWeight: 700, color: "rgba(255,255,255,0.78)", letterSpacing: "0.03em" }}>Research Papers <span style={{ color: "rgba(255,100,100,0.85)" }}>*</span></label>
                </div>
                <input
                  type="number"
                  required
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 28, padding: "10px 14px", borderRadius: 10, background: "rgba(192,132,252,0.05)", border: "1px solid rgba(192,132,252,0.1)" }}>
              <span style={{ color: "rgba(255,100,100,0.75)", fontSize: "14px", lineHeight: 1 }}>*</span>
              <span style={{ fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "14px", color: "rgba(255,255,255,0.28)" }}>All fields are mandatory to proceed</span>
            </div>

            <button onClick={() => {
              if (loopCount && paperCount && Number(loopCount) >= 1 && Number(paperCount) >= 1) setPhase("app");
            }} style={{ width: "100%", padding: "16px", borderRadius: 999, background: "rgba(147,51,234,0.28)", backdropFilter: "blur(30px) saturate(180%)", WebkitBackdropFilter: "blur(30px) saturate(180%)", border: "1px solid rgba(192,132,252,0.4)", borderTop: "1px solid rgba(220,180,255,0.55)", color: "rgba(220,180,255,0.95)", fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "14px", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", cursor: "pointer", boxShadow: "0 0 0 0.5px rgba(192,132,252,0.2) inset, 0 8px 32px rgba(147,51,234,0.38)", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ animation: "appReveal 0.6s ease both" }}>
          <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, 20%)", width: PANEL_W, display: "flex", flexDirection: "column", alignItems: "center", gap: 16, zIndex: 10 }}>
            ...
            <div style={{ textAlign: "center", pointerEvents: "none", width: "100%" }}>
              <div style={{ fontFamily: "'Yuji Syuku','Kaisei HarunoUmi',serif", fontSize: "clamp(1.8rem,3.5vw,2.8rem)", fontWeight: 700, letterSpacing: "0.22em", color: "#fff", textShadow: "0 0 28px rgba(168,85,247,0.95),0 0 70px rgba(147,51,234,0.6)", whiteSpace: "nowrap", textAlign: "center" }}>
                DAC AGENT
              </div>
            </div>
            ...
            <div style={{ width: "100%", height: PANEL_H, display: "flex", flexDirection: "column", borderRadius: 20, overflow: "hidden", boxShadow: "0 40px 100px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.08)" }}>
              ...
              <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
                ...
                <div style={{ ...liqBase, borderRadius: 0, borderRight: "1px solid rgba(255,255,255,0.07)", borderLeft: "none", borderTop: "none", width: 220, flexShrink: 0, display: "flex", flexDirection: "column", padding: "18px 12px", gap: 4, position: "relative", overflow: "hidden" }}>
                  ...
                  {NAV.map(item => (
                    <button key={item.id} onClick={() => setActive(item.id)} style={{ display: "flex", alignItems: "center", gap: 11, padding: "10px 12px", borderRadius: 12, border: "none", cursor: "pointer", width: "100%", textAlign: "left", background: active === item.id ? "rgba(147,51,234,0.22)" : "transparent", borderLeft: active === item.id ? "2px solid rgba(192,132,252,0.7)" : "2px solid transparent", backdropFilter: active === item.id ? "blur(20px)" : "none", transition: "all 0.18s", color: active === item.id ? "rgba(220,180,255,0.95)" : "rgba(255,255,255,0.38)", boxShadow: active === item.id ? "0 0 0 1px rgba(192,132,252,0.1) inset" : "none" }}>
                      ...
                      <span style={{ fontFamily: SF, fontSize: 13, fontWeight: active === item.id ? 600 : 400, letterSpacing: "0.01em" }}>{item.label}</span>
                    </button>
                  ))}
                  <div style={{ flex: 1 }} />
                  <div style={{ ...liqCard, borderRadius: 12, padding: "12px 14px", position: "relative", overflow: "hidden" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <span style={{ ...txt(0.7, 11, 600) }}>System Online <span style={{ ...txt(0.5, 14, 700), fontWeight: 700 }}>v1.0</span></span>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ ...liqBase, borderRadius: 0, borderLeft: "none", borderTop: "none", borderRight: "none", flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", position: "relative" }}>
                  ...
                  <div style={{ padding: "12px 24px 10px", borderBottom: "1px solid rgba(255,255,255,0.06)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ fontFamily: SFD, fontSize: 16, fontWeight: 900, color: "rgba(255,255,255,0.92)", letterSpacing: "0.04em", whiteSpace: "nowrap" }}>{titleMap[active]}</div>
                    <div style={{ ...txt(0.3, 16, 400), fontFamily: SF, whiteSpace: "nowrap" }}>{new Date().toLocaleDateString("en-US", { weekday: "short", month: "short", day: "2-digit", year: "numeric" })}</div>
                  </div>
              ...
              <div style={{ ...liqBase, borderRadius: "0", borderTop: "1px solid rgba(255,255,255,0.07)", height: 32, flexShrink: 0, display: "flex", alignItems: "center", padding: "0 18px", justifyContent: "space-between", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <span style={{ ...txt(0.3, 10, 400) }}>Sessions: 47</span>
                  <span style={{ ...txt(0.18, 10, 400) }}>|</span>
                  <span style={{ ...txt(0.3, 10, 400) }}>Tasks: 312</span>
                  <span style={{ ...txt(0.18, 10, 400) }}>|</span>
                  <span style={{ ...txt(0.3, 10, 400) }}>{new Date().toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}</span>
                </div>
              </div>
